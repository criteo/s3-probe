...
type consulClientMock struct {
	...
	ReadEndPoints           map[string][]probe2.S3Endpoint
...
func (cc *consulClientMock) GetServiceEndPoints(serviceName string, isGateway bool) (string, []probe2.S3Endpoint, error) {
	if cc.ServiceEndPointsError != nil {
		return "", []probe2.S3Endpoint{}, cc.ServiceEndPointsError
	}
...
func TestGetService(t *testing.T) {
	...
	consulClient.ReadEndPoints = map[string][]probe2.S3Endpoint{"myotherservice": {probe2.S3Endpoint{Name: "10.0.0.1"}, probe2.S3Endpoint{Name: "10.0.0.2"}}}
...
func s3ServicesFromStrings(strings []string) (s3Services []probe2.S3Service) {
	for i := range strings {
		s3Services = append(s3Services, probe2.S3Service{Name: strings[i]})
	}
	return s3Services
}
...
func TestGetWatchedServicesReturnTheCorrectEntries(t *testing.T) {
	...
	for _, serviceName := range testValues {
		watchedServices[serviceName] = watchedService{service: probe2.S3Service{Name: serviceName}}
	}
	w := Watcher{
...
func TestGetServicesToModifyHandleChangeOfEndpoint(t *testing.T) {
	servicesFromConsul := []probe2.S3Service{{Name: "s1", Endpoint: "10.0.0.1"}}
	servicesWatchedServices := []probe2.S3Service{{Name: "s1", Endpoint: "10.0.0.2"}}
	w := Watcher{}
	serviceToAdd, serviceToRemove := w.getServicesToModify(servicesFromConsul, servicesWatchedServices)
...
func TestGetServicesToModifyHandleChangeOfGatewayReadEndpoint(t *testing.T) {
	servicesFromConsul := []probe2.S3Service{{Name: "s1", Endpoint: "10.0.0.1", GatewayReadEnpoints: []probe2.S3Endpoint{{Name: "10.0.0.2"}, {Name: "10.0.0.3"}}}}
	servicesWatchedServices := []probe2.S3Service{{Name: "s1", Endpoint: "10.0.0.1", GatewayReadEnpoints: []probe2.S3Endpoint{{Name: "10.0.0.3"}}}}
	w := Watcher{}
	serviceToAdd, serviceToRemove := w.getServicesToModify(servicesFromConsul, servicesWatchedServices)
	if len(serviceToAdd) != 1 || len(serviceToRemove) != 1 {
		t.Errorf("getServicesToModify should have return s1 service in both serviceToRemove and serviceToAdd")
...
func TestGetServicesToModifyReturnsEmptyWhenNoChange(t *testing.T) {
	servicesFromConsul := []probe2.S3Service{{Name: "s1", Endpoint: "10.0.0.1", GatewayReadEnpoints: []probe2.S3Endpoint{{Name: "10.0.0.2"}, {Name: "10.0.0.3"}}}}
	servicesWatchedServices := []probe2.S3Service{{Name: "s1", Endpoint: "10.0.0.1", GatewayReadEnpoints: []probe2.S3Endpoint{{Name: "10.0.0.2"}, {Name: "10.0.0.3"}}}}
	w := Watcher{}
	serviceToAdd, serviceToRemove := w.getServicesToModify(servicesFromConsul, servicesWatchedServices)
...
func TestFlushOldProbesSendStopCommand(t *testing.T) {
	...
	w := Watcher{
		watchedServices: map[string]watchedService{"test": {service: probe2.S3Service{Name: "test"}, probeChan: controlChan}},
	}
